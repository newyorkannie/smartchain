    ...
    describe('calculateBlockTargetHash()', () => {
        ...
        describe('adjustDifficulty()', () => {
            ...
            it('decreases the difficulty for a slowly mined block', () => {
                ...
                ).toEqual(4); //5 - 1 bc 1 is the default
            });
        });
        ...
        describe('validateBlock()', () => {
            ...
            /*
            it('rejects when the proof of work requirement is not met', () => {
                Block.calculateBlockTargetHash = jest.fn().mockImplementation(() => {
                    return '0'.repeat(64);
            });

            const lastBlock = Block.genesis();
            const block = Block.mineBlock({ lastBlock, beneficiary: "beneficiary" });
            // Ensure the block is supposed to fail the test
            // ...

            return expect(Block.validateBlock({ lastBlock, block }))
                .rejects
            .toMatchObject({
                message: 'The block does not meet the proof of work requirement'
                });
