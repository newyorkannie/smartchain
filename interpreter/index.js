...
const AND = 'AND';
const OR = 'OR';
const JUMP = 'JUMP';    //jumps to line 18 -> programCounter
const JUMPI = 'JUMPI';
const STORE = 'STORE';  
const LOAD = 'LOAD';
  ...
  jump() {
    ...
    if (destination < 0 ||
        destination > this.state.code.length) {
            //throw error with a template string
  ...
  runCode(code) {
    ...
    while (this.state.programCounter < this.state.code.length) {
      ...
      if (this.state.executionCount > EXECUTION_LIMIT) {
        throw new Error(
            `Check for an infinite loop. Execution limit of ${EXECUTION_LIMIT} has exceeded`
        );
      }
...
//module.exports = { Interpreter, OPCODE_MAP };
...
// //this will start by pushing on the 6
// //then it will jump to 0 and loop around
// code = [PUSH, 6, JUMP, PUSH, 0, JUMP, PUSH, 'jump successful', STOP];
// result = new Interpreter().runCode(code);
// console.log('Result of JUMP:', result); 

//JUMP example with infinite loop
// code = [PUSH, 0, JUMP, PUSH, 'jump successful', STOP];
// result = new Interpreter().runCode(code);
// console.log('Result of JUMP:', result); 
...
// //TEST INFINTE LOOP
// code = [PUSH, 0, JUMP, STOP];   //this is constantly pushing to 0 all the time
// try {
//     new Interpreter().runCode(code);
// } catch (error) {
